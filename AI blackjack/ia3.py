...
for episode in range(1,num_episodes+1):
    ...
    if episode % 10000 == 0:
        ...
        print(f"Loading: {int(episode/10000)}%, Mean_win: {round(stat.mean(results), 3)}")
        results=[]
...
for episode in range(1,num_episodes+1):
    ...
    while not game.over:
        q_values = [agent.Q.get((state, a), 0) for a in agent.actions]
        max_q = max(q_values)
        actions_with_max_q = [a for a, q in zip(agent.actions, q_values) if q == max_q]
        action = np.random.choice(actions_with_max_q)
        # Appliquer l'action choisie dans le jeu
        game.choice(action)
...
with open('q_table_test.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    # Écrire l'en-tête
    writer.writerow(['État', 'Action', 'Valeur Q'])
    # Écrire les données
    for (etat, action), valeur_q in Q.items():
        writer.writerow([etat, action, valeur_q])